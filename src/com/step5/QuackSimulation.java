...
public class QuackSimulation {
	...
	void methodB(QuakBehavior quackBehavior) {
		System.out.println("methodB "+quackBehavior);
	}

	public static void main(String[] args) {
		//선언부와 생성부에 타입이 다른거 - 권장사항 - 범위가 더 넓은 걸 사용하면 범위가 더 좁은 여러 개의 구현체 클래스를 담을 수 있으니까
		//유지보수 시에 코드의 수정을 최소화 하는데 역할이 있다.
		QuackSimulation qs = new QuackSimulation();
		QuakBehavior quackBehavior = new Squeak();
		MuteQuack quackBehavior1 = new MuteQuack();
		//qs.methodA(quackBehavior);
		if(quackBehavior instanceof Squeak) {
			System.out.println("나는 Squak 타입이다.");			
		}
		if(quackBehavior instanceof MuteQuack) {
			System.out.println("나는 MuteQuack 타입이다.");			
		}
		if(quackBehavior instanceof Quak) {
			System.out.println("나는 Quack 타입이다.");			
		}
		qs.methodB(quackBehavior1);
		Quak quackBehavior2 = new Quak();
