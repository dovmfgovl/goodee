...
//인터페이스는 여러개를 implements할 수 있다.(추상클래스, 인터페이스는 설계관점에서 중요함 - 특징, 컨벤션)
public class DeptManager3 extends JFrame implements ActionListener{
	...
	PreparedStatement 	pstmt 	= null;//Connection생성돼야 PreparedStatement메모리 로딩됨
	...
	JTable jt_dept = new JTable(dtm_dept); //데이터를 행과 열로 구성된 테이블 형식으로 보여주는 컴포넌트
	JScrollPane jsp_dept = new JScrollPane(jt_dept); //JTable을 JScrollPane에 붙여 스크롤함
	...
	//접근제한자가 없는 경우는 friendly 상태라 하는데 다른 패키지에서 접근이 불가능한 상태임
	//public > protected(패키지가 다르더라도 서로 상속관계면 접근가능함) > friendly(패키지 다르면 접근불가) > private
	public DeptManager3(){
	...
	public List<Map<String,Object>> getMapList(){//2개 이상의 테이블 조인시 - 이것으로 연습 더 많이
		...
		//StringBuffer는 스레드안전, StringBuilder 불안전하다 - 인터셉트를 당할 수 있다.  - 로컬이면 괜찮아.
	...
	public static void main(String[] args) {
		JFrame.setDefaultLookAndFeelDecorated(true); //JFrame 객체 선언 전에 설정 해야 효과 적용됨
