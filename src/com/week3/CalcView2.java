...
public class CalcView2 {
	//선언부
	//this - 현재 메모리에 로딩 되어있는 클래스 자신을 의미함
	CalcLogic cLogic = new CalcLogic(this); //생성자의 파라미터 자리를 통해서 넘긴다.
	CalcEvent cEvent = new CalcEvent(this);
	JFrame jf = new JFrame();
	JTextField jtf_display = new JTextField("0");
	JPanel jp = new JPanel();
	//위치를 선택하는 기준이 있나요? actionPerformed 안에서 사용하려면 전역변수여야 하기에
	...
	public void initDisplay() {
		...
		jf.add("Center",jp);
		jf.setSize(400, 400);
		jf.setVisible(true); //true이면 활성화, false이면 비활성화 - 둘 다 메모리에 살아있다.
	}
	//메인메소드
