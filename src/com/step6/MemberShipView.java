...
public class MemberShipView extends JDialog implements ActionListener{
    ...
    //insert here
    //우편번호 검색기 클래스를 전역변수 자리에서 생성한다 - MemberShipView가 생성될 때 생성됨
    //파라미터 자리에 this를 쓸 수 있는 건 이 어플리케이션을 실행했을 때 비로소 객체가 로딩됨
    ZipCodeView zcView = new ZipCodeView(this); //this는 현재 로딩 중인 원본이다. 
    //생성자
    ...
    public static void main(String[] args) {
    	MemberShipView ms = new MemberShipView(); //전역변수 자리에 인스턴스화 한 객체들이 비로소 이때 로딩이 됨
        ms.initDisplay();
    }
    @Override
