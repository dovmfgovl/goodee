...
public class CustomerManager {
	 ...
	 * select(1건)  - rcVO or Map(1건) - pcVO - 1이면 성공, 0이면 수정 실패 판정
	 ...
	 * @param int
	 * @return	true이면 성공, false이면 실패 - 메소드 선언시에 파라미터 타입에 대한 선택과 리턴타입에 대한 선택 기준
	 ***************************************************************/
	public boolean customerDelete(int c_id) {//VO를 굳이 사용하지 않고 int 써도 괜찮다. 위 WHERE 자리에 들어가니까
		boolean isOk = false;
		return isOk;
	}
	/****************************************************************
	 ...
	 * @param c_id
	 * @return CustomerVO(한 테이블 일때 사용) or Map(두 테이블 이상이어서 조인이 불가피 할때 사용)
	 ****************************************************************/
	...
	/*****************************************************************
	 * 고객정보 전체보기 구현 - 파라미터가 필요없다. - 왜냐하면 조건절이 필요없으니까
