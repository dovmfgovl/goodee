...
class List4{
	...
	//사용자로부터 두 개의 정보를 입력 받아온다(UI, UX, View 계층에서)
	boolean rankUpdate(int index, int value) {
		boolean isOk = false;
		Integer it = rank.set(index, value);
		//44번을 경유하는지 아니면 스킵하게 되는 건지 짐작 가능함
		int x = it; //오토박싱이 제공되고 있어서 원시형 타입과 래퍼 클래스 타입 사이에 형전환 자동으로 해 줌
		System.out.println(x + ", " + value);
		//"1".equals("1");
		System.out.println(it == value);
		if(it != null) {
		//if(it == value) { //사용자로부터 받아온 값과 set메소드의 리턴값으로 받아온 값을 비교하기
...
public class ListMain4 {
	...
	public static void main(String[] args) {
		...
		int ivalue = Integer.parseInt(value); //String -> int 바꿔줘(기능, 처리, 메소드힘)
		List4 l4 = new List4();
		l4.init();
