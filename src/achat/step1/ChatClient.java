...
public class ChatClient extends JFrame implements ActionListener {
	...
	//소켓 관련 초기화
		public void init() {
			try {
				//서버측의 ip주소 작성하기
				socket = new Socket("127.0.0.1",3002); //127.0.0.1 로컬 호스트를 의미하는 상수값 //ServerSocket -> socket -> C.S.T client
				...
				ChatClientThread tct = new ChatClientThread(this); //아직 일이 없다
				tct.start();
			} catch (Exception e) {
	...
	public void initDisplay() { //(키위/ 양파/ 토마토)님의 창 - 3명이 바뀌니까 지역변수? 전역변수? - 전역변수..!
	...
	public void actionPerformed(ActionEvent e) {
		//말하기 구현 -> oos.writeObject(''200 | kiwi | tomato | 오늘 스터디 할까?"); //프로토콜 설계
		...
		if(jtf_msg == obj) {
			try {
				...
				//메시지를 서버로 전송하고 나면 JTextField 적힌 문자열은 지운다. 
				jtf_msg.setText("");
			} catch (Exception e2) {
