...
public class BoardVO {
	...
	private int result = -1; //0과 1이 아닌 값들이 훼손되지 않도록 -1 선언
	...
	public void setResult(int result) { //1 또는 0
		...
		//public으로 변수를 선언하여 직접 사용할 때 보다 메소드를 통해서 값을 사용함으로
		//입력값에 대한 유효성 체크를 할 수 있다는 장점이 있다.
		if(result == 0 | result == 1) { //입력 값에 대한 유효성 체크를 동반할 수도 있다.
			isValid = true;
		} else {
			isValid = false;
		}
